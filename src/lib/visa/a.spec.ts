...
function getIds(criteria: Criteria[]) {
  return criteria.map(it => it.id)
}
...
describe('Visa type A point simulation', () => {
  describe('categories', () => {
    describe('academic background', () => {
      it('doctor degree', () => {
        const q: Qualifications = { degree: 'doctor' }

        const { matches, points } = calculatePointsForVisaA(q)

        expect(points).toBe(30)
        expect(getIds(matches)).toIncludeSameMembers(['doctor'])
      })

      it('a dual degree should give bonus', () => {
        const q: Qualifications = { degree: 'master', dual_degree: true }

        const { matches, points } = calculatePointsForVisaA(q)

        expect(points).toBe(25)
        expect(getIds(matches)).toIncludeSameMembers(['dual_degree', 'master'])
      })
    })

    describe('professional career', () => {
      it('7 years of experience', () => {
        const q: Qualifications = { experience: 7 }

        const { matches, points } = calculatePointsForVisaA(q)

        expect(points).toBe(15)
        expect(getIds(matches)).toIncludeSameMembers(['7y_plus'])
      })

      it('5 years of experience', () => {
        const q: Qualifications = { experience: 5 }

        const { matches, points } = calculatePointsForVisaA(q)

        expect(points).toBe(10)
        expect(getIds(matches)).toIncludeSameMembers(['5y_plus'])
      })

      it('3 years of experience', () => {
        const q: Qualifications = { experience: 3 }

        const { matches, points } = calculatePointsForVisaA(q)

        expect(points).toBe(5)
        expect(getIds(matches)).toIncludeSameMembers(['3y_plus'])
      })

      it('1 years of experience', () => {
        const q: Qualifications = { experience: 1 }

        const { matches, points } = calculatePointsForVisaA(q)

        expect(points).toBe(0)
        expect(getIds(matches)).toIncludeSameMembers([])
      })
    })
